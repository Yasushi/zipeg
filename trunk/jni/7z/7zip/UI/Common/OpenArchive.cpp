...
#ifdef FORMAT_ARJ
#include "../../Archive/Arj/ArjHandler.h"
#endif
...
#ifdef FORMAT_LZH
#include "../../Archive/Lzh/LzhHandler.h"
#endif
...
#ifdef FORMAT_RPM
#include "../../Archive/Rpm/RpmHandler.h"
#endif

#ifdef FORMAT_CPIO
#include "../../Archive/Cpio/CpioHandler.h"
#endif
    ...
    IArchiveOpenCallback *openArchiveCallback)
{
  *archiveResult = NULL;
  ...
  for(i = 0; i < orderIndices.Size(); i++)
  {
    ...
    #ifdef FORMAT_LZH
    if (archiverInfo.Name.CompareNoCase(L"Lzh") == 0) {
//	  printf("new LzhHandler() %d\n", archiverInfo.Name.CompareNoCase(L"Lzh") == 0); fflush(stdout);
      archive = new NArchive::NLzh::CHandler;
    }
    #endif
    ...
    #ifdef FORMAT_ARJ
    if (archiverInfo.Name.CompareNoCase(L"Arj") == 0) {
      archive = new NArchive::NArj::CHandler();
    }
    #endif

    #ifdef FORMAT_CHM
    if (archiverInfo.Name.CompareNoCase(L"Chm") == 0) {
      archive = new NArchive::NChm::CHandler();
    }
    #endif
    ...
    #ifdef FORMAT_RPM
    if (archiverInfo.Name.CompareNoCase(L"Rpm") == 0) {
      archive = new NArchive::NRpm::CHandler();
    }
    #endif

    #ifdef FORMAT_CPIO
    if (archiverInfo.Name.CompareNoCase(L"Cpio") == 0) {
      archive = new NArchive::NCpio::CHandler();
    }
    #endif

    #ifdef FORMAT_ISO
    if (archiverInfo.Name.CompareNoCase(L"Iso") == 0) {
      archive = new NArchive::NIso::CHandler();
    }
    #endif

    #ifdef FORMAT_NSIS
    if (archiverInfo.Name.CompareNoCase(L"Nsis") == 0) {
      archive = new NArchive::NIso::CHandler();
    }
    #endif

    #ifdef FORMAT_RAR
    if (archiverInfo.Name.CompareNoCase(L"Rar") == 0) {
    ...
    {
      badResult = result;
      if(result == E_ABORT || result == E_INVALID_PASSWORD)
        break;
      continue;
    }
