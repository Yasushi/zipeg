...
#else
#error "_UNICODE is defined. Some code changes are needed"
#endif
...
// __asm int 3
	UString uname;
	AString aname(filename);
	if (ConvertUTF8ToUnicode(aname, uname)) {		
            UString longname = L"\\\\?\\" + uname; // allows 32K chars unicode filenames
	    _fd = ::CreateFileW(longname, dwDesiredAccess, dwShareMode, NULL, dwCreationDisposition, dwFlagsAndAttributes, NULL);
            if (_fd == INVALID_HANDLE_VALUE) { // UNC names
    	        _fd = ::CreateFileW(uname, dwDesiredAccess, dwShareMode, NULL, dwCreationDisposition, dwFlagsAndAttributes, NULL);
            }
        } else {
		_fd = ::CreateFileA(filename, dwDesiredAccess, dwShareMode, NULL, dwCreationDisposition, dwFlagsAndAttributes, NULL);
	}
...
#ifdef _WINDOWS
  DWORD hi = 0;
  DWORD lo = GetFileSize(_fd, &hi);
  length = (((UINT64)hi) << 32) | lo;
  return (__int64)length != (__int64)-1;
#else
  if (_fd == -1)
  {
...
{
#ifdef _WINDOWS
    return ::SetFilePointerEx(_fd, *(LARGE_INTEGER*)&distanceToMove, (LARGE_INTEGER*)&newPosition, moveMethod) != 0;
#else
  if (_fd == -1)
  {
     SetLastError( ERROR_INVALID_HANDLE );
...
{
#ifdef _WINDOWS
    return ::ReadFile(_fd, buffer, bytesToRead, (DWORD*)&bytesRead, NULL) != 0;
#else
  if (_fd == -1)
  {
     SetLastError( ERROR_INVALID_HANDLE );
...
{
#ifdef _WINDOWS
    return ::SetFileTime(_fd, lpCreationTime, lpLastAccessTime, lpLastWriteTime) != 0;
#else
  LARGE_INTEGER  ltime;
  DWORD dw;
...
{
#ifdef _WINDOWS
    return ::WriteFile(_fd, buffer, bytesToWrite, (DWORD*)&bytesWritten, NULL) != 0;
#else
  if (_fd == -1)
  {
     SetLastError( ERROR_INVALID_HANDLE );
