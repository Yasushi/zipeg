...
public class UniversalDetector {
    ...
    public void handleData(final byte[] buf, int offset, int length) {
        ...
        for (int i = offset; i < maxPos; ++i) {
            int c = buf[i] & 0xFF;
            if ((c & 0x80) != 0 && c != 0xA0) {
                if (this.inputState != HIGHBYTE) {
                    this.inputState = HIGHBYTE;
    ...
    public void dataEnd() {
        ...
        } else if (this.inputState == ESC_ASCII) {
            // do nothing
        } else {
            // do nothing
        }
